...
/**
 * GUI to show the Othello game and to listen for input for the user/human
 * player. When it is the user's turn,
 * (s)he clicks on the place where (s)he wants to place a token, and when it is
 * the computer's turn, the
 * player needs to click anywhere in the frame to make the computer take it's
 * turn. The user is made aware
 * of any illegal moves, or when (s)he - or the computer - have to pass because
...
public class OthelloGUI extends JComponent implements MouseListener {
	 ...
	 * Initializes game
	 * 
	 * @param player1     The AI for player 1 (black); this argument is ignored if
	 *                    there is indeed a human player.
	 * @param player2     The AI for player 2 (white).
	 * @param size        The number of rows and columns of the game board. Should
	 *                    be an
	...
	public OthelloGUI(IOthelloAI ai1, IOthelloAI ai2, int size, boolean humanPlayer) throws IOException {
		...
		// uncomment to make the AI play without you clicking the mouse
		// this.autoPlay();
					...
					if (humanPlayer) { // If there is a human involved, (s)he needs to know this
						...
						if (canMoveAfterwards) {
							String message = currentPlayer == 1
									? "Your opponent has no legal moves. It is your turn again."
